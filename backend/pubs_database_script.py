    ...
    if len(result_for_user) == 0:
        ...
        with st.container(height=500, border=False):
            ...
            for index, row in result_for_user.iterrows():
                if row['type'] == 'journal':
                    row_string = rf"- {row['authors']} ({row['year']}). {row['title']}. *{row['journal_name']}*, *{str(int(row['journal_vol_no']))}*({str(int(row['journal_issue_no']))}){', '+str(row['journal_page_range']) if not pd.isna(row['journal_page_range']) else ''}. {row['doi_or_url']}"
