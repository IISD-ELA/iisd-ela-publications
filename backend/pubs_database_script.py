...
def combined_search(data, 
    ...
    queries = [ (data_type_query,
                ...
                (iisd_ela_rel_query,
                    data.apply(lambda row:
                                    row['Unnamed: 23'].astype(str).str.contains(iisd_ela_rel_query).any(),
                                                                                             axis=1)),
                (general_search_query,
                    data.apply(lambda row: 
...
with col1: 
    ...
    rel_to_iisd_ela_query = st.multiselect(r"$\bold{Publication's} \: \bold{relationship} \: \bold{to} \: \bold{IISD} - \bold{ELA}$",
